...
try:
    conn = psycopg2.connect(database="charing", user="postgres", password="postgres", host="127.0.0.1",
                            port="5432")
    cur = conn.cursor()
    cur.execute("SELECT email,first_mail_content FROM users where pair_email is NULL and sex = FALSE ORDER BY register_time ASC limit 1")
    ...
    cur.execute("SELECT email,first_mail_content FROM users where pair_email is NULL and sex = True ORDER BY register_time ASC limit 1")
    rows = cur.fetchall()  # all rows in table
